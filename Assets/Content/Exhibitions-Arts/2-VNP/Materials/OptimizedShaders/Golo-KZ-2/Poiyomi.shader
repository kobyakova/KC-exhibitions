Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/d688bfaa4216163468f2ebb2533fa9eb"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if ((1.0 /*_LightingAdditiveType*/) == 1)
				{
					...
					#endif
					if ((0.0 /*_LightingAdditiveGradientEnd*/) == (0.0 /*_LightingAdditiveGradientStart*/)) (0.0 /*_LightingAdditiveGradientEnd*/) += 0.001;
					poiLight.rampedLightMap = smoothstep((0.0 /*_LightingAdditiveGradientEnd*/), (0.0 /*_LightingAdditiveGradientStart*/), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.0 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.0 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				...
				for (int index = 0; index < 4; index++)
				{
					...
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * (0.5 /*_LightingAdditivePassthrough*/) * poiLight.vAttenuation[index], smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.0 /*_LightingAdditiveGradientEnd*/), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				{
					if ((12.0 /*_FlipbookFPS*/) != 0)
					{
						currentFrame = (_Time.y / (1 / (12.0 /*_FlipbookFPS*/))) % totalFrames;
						if ((0.0 /*_FlipbookStartAndEnd*/))
						{
				...
				if ((1.0 /*_FlipbookIntensityControlsAlpha*/))
				{
				...
				poiFragData.baseColor = poiFragData.baseColor + flipBookPixel.rgb * (0.236 /*_FlipbookAdd*/) * flipBookMask * flipbookAlpha;
				poiFragData.baseColor = poiFragData.baseColor * lerp(1, flipBookPixel.rgb, flipBookPixel.a * float4(1,1,1,1).a * flipBookMask * (1.0 /*_FlipbookMultiply*/) * flipbookAlpha);
				float flipbookEmissionStrength = (0.69 /*_FlipbookEmissionStrength*/);
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
